...
public final class FuncTable(){
    ...
    public void addFunc(String s, int line, Type type, int id_escopo, List <Type> parameters){
        Key key = new Key (s, id_escopo);
        Entry entry = new Entry (s,line,type,id_escopo,parameters);
        table.put(key,entry);
    }
    ...
    public String toString() {
		...
		for (Map.Entry mapElement : table.entrySet()) {
			...
			f.format("Entry %d -- name: %s, line: %d, type: %s, id_escopo: %d, parameters_type:%s\n",
	                 getName(key), getLine(key), getType(key).toString(), 
                     getIdEscopo(key),Array.toString(getParameters().toArray()));
		}
		f.close();
		return sb.toString();
	}
    ...
    private static final class Entry {
		...
		Entry(String name, int line, Type type, int id_escopo, List <Type> parameters) {
			this.name = name;
			this.line = line;
			this.type = type;
